...
begin
  ...
  if new.max_players > 8
    and new.start_date is not null
    and coalesce(new.time_per_level, 0) > 0
    and now() >= new.start_date + (new.time_per_level * 10) * interval '1 minute'
  then
    raise exception 'Les inscriptions sont fermées après le niveau 10.'
      using
        errcode = 'P0001',
        hint = 'Registration is closed once level 10 has ended.';
  end if;
